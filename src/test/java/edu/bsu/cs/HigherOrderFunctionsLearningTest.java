...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(n -> n.startsWith("b")).collect(Collectors.toList());
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(n -> n.substring(0,1)).distinct()
                .collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(n -> n.endsWith("2")).map(Object::toString)
    ...
    public void testCountWhitelisted() {
        ...
        long actual = input.filter(revision -> revision.user.contains(whitelist.get(0)) ||
                revision.user.contains(whitelist.get(1)) ||
                revision.user.contains(whitelist.get(2))).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(revision -> !revision.user.matches("(?i).*" + "bot" + ".*")).count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(revision ->
                ZonedDateTime.ofInstant(revision.timestamp, ZoneId.of("UTC")).getMonth() == Month.FEBRUARY).count();
        int expected = 9;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.collect(Collectors.groupingBy(revision ->
                ZonedDateTime.ofInstant(revision.timestamp, ZoneId.of("UTC")).getMonth(), Collectors.counting()));
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = Collections.max(input.collect(Collectors.groupingBy(revision ->
                ZonedDateTime.ofInstant(revision.timestamp, ZoneId.of("UTC")).getMonth(),
